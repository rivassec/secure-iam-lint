...
const STORIES = [
  { id: 'S1-dataexfil-arn', maxIter: 8, focus: `HIGH fail-open: DATA-EXFIL / resourceIsBroad is blind to broad ARN-wildcard resources. A prior enumerative attempt (isBroadArnResource in ${ENGINE}/rules.js, a chain of startsWith('*')/segment checks) is LEAKY - the adversarial hunter defeated it with new spellings of the SAME class. DO NOT continue enumerating shapes. REPLACE the broadness test with a SINGLE SEMANTIC one:

A resource is BROAD iff its value, interpreted as an AWS resource GLOB (case-sensitive over the WHOLE string, '*' = any run incl. empty, '?' = exactly one char), matches a fixed BATTERY of diverse canonical PROBE ARNs that span multiple ACCOUNTS and multiple SERVICES/resource-types (e.g. s3 object in acct A, s3 object in acct B, iam role in acct C, kms key in acct D, sqs queue, an s3 access-point object, a bare non-arn resource). Broad = it matches probes across >=2 distinct accounts AND is not pinned to one specific concrete resource (i.e. it reaches across the account or service boundary, or matches a whole resource collection). Implement one small pure arnGlobMatch(pattern, probe) (reuse/adapt the existing linear glob matcher; treat '?' as one char) and a frozen PROBE list; wire resourceIsBroad() + the ruleDataExfil breadth gate + the ruleDataReadScoped fallback + the WILDCARD-RESOURCE consumer to this one predicate.

This MUST close ALL of these residual fail-opens the hunter reproduced (each currently analyze()->0 findings, must now fire DATA-EXFIL:high or WILDCARD-RESOURCE:high on BOTH analyze() and scan()): the NON-arn-prefixed globs '*/*', '?*', '**', '*:*', 'arn*'; the '?'-wildcard forms arn:aws:s3:::?*/*, arn:aws:?*, arn:?:?:?:?:?; resource-type-prefixed arn:aws:s3:...:accesspoint/*/object/*, arn:aws:iam::*:role/*, s3-outposts outpost/op/bucket/* (terminal collection wildcard); and leading-whitespace ' arn:aws:s3:::*'. Add a fixtures/must-warn/ entry for EACH.

CRITICAL - do NOT over-flag (the qa/false-positive critic + negative corpus guard this): a resource scoped to ONE specific resource must stay quiet - arn:aws:s3:::my-bucket/*, arn:aws:s3:::my-bucket/prefix/*, arn:aws:iam::123456789012:role/app-team/*, arn:aws:kms:us-east-1:123456789012:key/abcd. Keep the whole negative corpus green.` },
