...
- [x] escalation-deny.js + escalation-finding.js DONE (deny-first ordering; isStarResource + applyDenyToActions + ESCALATIONS deps)
- [~] (OLD) escalation-principal-pins.js DEFERRED (extraction broke 50 tests in the full-suite run w/ 0 madge cycles - likely a load-order/shared-state coupling; needs careful manual handling, left in escalation.js) (principalPinsOf, principalPinsOfMemo, pinsJointlySatisfiable, principalConditionsSatisfiable, constraintContains, keyConstraintsSatisfiable) -- memo is a PARAM, safe
